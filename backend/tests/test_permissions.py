...
def test_regular_user_cannot_list_users(client, db_session):
    ...
    response = client.get("/api/users", cookies=auth_cookies(client, "simple@test.example"))
    assert response.status_code == 403


def test_manager_can_list_users(client, db_session):
    create_user(db_session, "manager@test.example", "Responsable IT")
    response = client.get("/api/users", cookies=auth_cookies(client, "manager@test.example"))
...
def test_only_admin_can_create_role(client, db_session):
    ...
    response_manager = client.post("/api/roles", json={"name": "Rôle Test"}, cookies=auth_cookies(client, "manager2@test.example"))
    assert response_manager.status_code == 403

    response_admin = client.post("/api/roles", json={"name": "Rôle Test"}, cookies=auth_cookies(client, "admin@test.example"))
