...
def test_create_ticket_generates_reference(client, db_session):
    ...
    headers = auth_headers(client, "demandeur@test.example")

    response = client.post("/api/tickets", json=_ticket_payload(db_session), headers=headers)
...
def test_technician_can_take_unassigned_ticket(client, db_session):
    ...
    ticket = client.post("/api/tickets", json=_ticket_payload(db_session), headers=auth_headers(client, "demandeur2@test.example")).json()
...
def test_technician_cannot_assign_ticket_to_someone_else(client, db_session):
    ...
    ticket = client.post("/api/tickets", json=_ticket_payload(db_session), headers=auth_headers(client, "demandeur3@test.example")).json()

    response = client.post(
        f"/api/tickets/{ticket['id']}/assign",
        json={"technician_id": autre_technicien.id},
        headers=auth_headers(client, "technicien2@test.example"),
    )
...
def test_resolve_ticket_sets_solution_and_status(client, db_session):
    ...
    ticket = client.post("/api/tickets", json=_ticket_payload(db_session), headers=auth_headers(client, "demandeur4@test.example")).json()

    response = client.post(
        f"/api/tickets/{ticket['id']}/resolve",
        json={"solution": "Remplacement du câble d'alimentation."},
        headers=auth_headers(client, "technicien4@test.example"),
    )
    assert response.status_code == 200
    body = response.json()
...
def test_regular_user_cannot_view_others_tickets(client, db_session):
    ...
    ticket = client.post("/api/tickets", json=_ticket_payload(db_session), headers=auth_headers(client, "proprietaire@test.example")).json()

    response = client.get(f"/api/tickets/{ticket['id']}", headers=auth_headers(client, "intrus@test.example"))
    assert response.status_code == 403
...
def test_only_user_role_can_create_ticket(client, db_session):
    ...
    response_user = client.post("/api/tickets", json=payload, headers=auth_headers(client, "demandeur5@test.example"))
    assert response_user.status_code == 201

    for email in ("technicien5@test.example", "manager5@test.example", "admin5@test.example"):
        response = client.post("/api/tickets", json=payload, headers=auth_headers(client, email))
